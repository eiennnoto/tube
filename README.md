...
## Classroom動画について
Google Classroomの `/u/0/n/player?v=...` は、外部サイトのiframe内で正常に表示できない場合があります。
そのため動画ページでは、Classroomのプレイヤーを新しいタブで直接開く方式にしています。
これはブラウザ側のiframe制約を避けるためで、Render側のHTMLを変更するだけでClassroomのプレイヤーを確実に埋め込めるものではありません。
