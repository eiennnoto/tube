...
const els = {
  ...
  addCurrentBtn: document.querySelector("#addCurrentBtn"),
...
async function openVideo(id, pushUrl = true) {
  ...
  state.currentId = id;
  showPage("video");
  els.addCurrentBtn.hidden = false;
  ...
  try {
    ...
    // The requested behavior: use the iframe src discovered on the video page.
    // If no iframe src could be extracted, fall back to the PCK page itself.
    els.player.src = data.iframeSrc || data.pageUrl;
  } catch (error) {
    console.error(error);
...
function refreshSaveButtons() {
  const saved = isSaved(state.currentId);
  els.saveVideoBtn.textContent = saved
    ? "✓ 動画リストに保存済み"
    : "＋ 動画リストに追加";
  els.addCurrentBtn.textContent = saved
    ? "✓ 保存済み"
    : "＋ リストに追加";
}
...
els.saveVideoBtn.addEventListener("click", () => {
  if (state.currentId) addToList(state.currentId);
});
